...
public class Examen2Version2_Guzman {
	...
	public static void main(String[] args) throws Exception {
		...
		int bestI = 0;
		for (int i = 0; i<= 5; i++) {
			polyKernel.setExponent((double)i);
			SMO.setKernel(polyKernel);
			evaluator.crossValidateModel(SMO, filteredData, 3, new Random(3));
			if (topFMeasure <= evaluator.weightedFMeasure()) {
				topFMeasure = evaluator.weightedFMeasure();
				bestI = i;
			}
		}
		polyKernel.setExponent((double)bestI);
		SMO.setKernel(polyKernel);
		evaluator.crossValidateModel(SMO, filteredData, 4, new Random(3));
		
		// Guardado del modelo
		SMO.buildClassifier(filteredData);
		saveModel(SMO, modelPath);
		
		// Evaluación del clasificador		
		String results = testHoldOut(SMO, filteredData, 70) + "\n\n" + testNoHonesta(SMO, filteredData);
